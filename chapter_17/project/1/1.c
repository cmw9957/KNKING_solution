//16.3의 inventory.c 프로그램을 inventory 배열을 동적할당을 이용해 수정 
...
int main(void){
	char code;
	struct part *inventory = malloc(sizeof(struct part) * mul);
	if(inventory == NULL){
		printf("No space!\n");
		return -1;
	}
...
void insert(struct part *inventory, int *num_parts){
	int part_number;
	if(size >= (int)sizeof(struct part) * mul){
		printf("REALLOC!\n");
		mul *= 2;
		inventory = (struct part *)realloc(inventory, mul * sizeof(struct part));
	}
